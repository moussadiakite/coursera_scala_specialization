...
object TimeUsage {
  ...
  def row(line: List[String]): Row = {
    val convertedList: Seq[Any] = line.head :: line.tail.map(_.toDouble)
    Row.fromSeq(convertedList)
  }
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    columnNames.foldLeft((List[Column](), List[Column](), List[Column]())){
      case (acc, columnName) => columnName match {
        case c if List("t01", "t03", "t11", "t1801", "t1803").exists(c.startsWith(_)) => (col(columnName)::acc._1, acc._2, acc._3)
        case c if List("t05", "t1805").exists(c.startsWith(_)) => (acc._1, col(columnName)::acc._2, acc._3)
        case c if List("t02", "t04", "t06", "t07", "t08", "t09", "t10", "t12", "t13", "t14", "t15", "t16",
      "t18").exists(c.startsWith(_)) => (acc._1, acc._2, col(columnName)::acc._3)
        case _ => acc
      }
    }
  }
  ...
  def timeUsageSummary(
    ...
    val workingStatusProjection: Column = when($"telfs" >= 1 && $"telfs" < 3, "working").
      otherwise("not working").as("workingStatusProjection")
    val sexProjection: Column = when($"tesex" === 1, "male").
      otherwise("female").as("sexProjection")
  ...
  def timeUsageGrouped(summed: DataFrame): DataFrame = {
    summed.select($"workingStatusProjection".as("working"),
     ...
     .agg(round(avg($"primaryNeedsProjection"), 1).as("primaryNeeds"),
  ...
  def timeUsageGroupedSqlQuery(viewName: String): String = {
    val selectCols = "workingStatusProjection as working, sexProjection as sex, ageProjection as age"
    val cols = "workingStatusProjection, sexProjection, ageProjection"
    "SELECT " + selectCols + ", round(avg(primaryNeedsProjection), 1) as primaryNeeds, " +
                             "round(avg(workProjection), 1) as work, " +
                             "round(avg(otherProjection), 1) as other FROM " +
    viewName + " GROUP BY " + cols + " ORDER BY " + cols
  }
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    summed.groupByKey(tuRow => (tuRow.working, tuRow.sex, tuRow.age)).
      ...
      map{
        case ((working, sex, age), primaryNeedsAvg, workAvg, otherAvg) => TimeUsageRow(working, sex, age, primaryNeedsAvg, workAvg, otherAvg)
      }.orderBy($"working", $"sex", $"age")
  }
}
