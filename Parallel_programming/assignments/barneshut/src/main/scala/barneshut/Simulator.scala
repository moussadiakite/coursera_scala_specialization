...
class Simulator(val taskSupport: TaskSupport, val timeStats: TimeStatistics) {
  ...
  def updateBoundaries(boundaries: Boundaries, body: Body): Boundaries = {
    if(body.x < boundaries.minX || body.x > boundaries.maxX || body.y < boundaries.minY || body.y > boundaries.maxY) {
  ...
  def XIntersect(a: Boundaries, b: Boundaries): Boolean = {
    a.minX <= b.maxX && a.minX >= b.minX
  }

  def YIntersect(a: Boundaries, b: Boundaries): Boolean = {
    a.minY <= b.maxY && a.minY >= b.minY
  }

  def includes(includes: Boundaries, isIncluded: Boundaries): Boolean = {
    isIncluded.minX >= includes.minX && isIncluded.minX <= includes.maxX && isIncluded.maxX <= includes.maxX &&
    isIncluded.minY >= includes.minY && isIncluded.minY <= includes.maxY && isIncluded.maxY <= includes.maxY
  }
  ...
  def computeSectorMatrix(bodies: Seq[Body], boundaries: Boundaries): SectorMatrix = timeStats.timed("matrix") {
    ...
    parBodies.aggregate(new SectorMatrix(boundaries, SECTOR_PRECISION))(
      {case (sectorMatrix, b) => (sectorMatrix += b)},
      {case (sectorMatrix1, sectorMatrix2) => sectorMatrix1.combine(sectorMatrix2)}
    )
  }
