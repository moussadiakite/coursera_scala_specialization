...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweetedAcc(acc: Tweet): Tweet = {
    val mRetweeted = if(elem.retweets > acc.retweets) elem else acc
    val leftMRetweeted = left.mostRetweetedAcc(mRetweeted)
    val rightMRetweeted = right.mostRetweetedAcc(mRetweeted)
    if(leftMRetweeted.retweets > rightMRetweeted.retweets) leftMRetweeted else rightMRetweeted
  }
