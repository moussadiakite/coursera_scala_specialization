...
import FloatOps._
  ...
  test("Leaf.insert(b) should return a new Fork with size > minimumSize") {
    ...
    inserted match {
      case fork @ Fork(_, _, _, _) =>
        assert(fork.centerX == 17.5f, s"$fork.centerX should be 17.5f")
        assert(fork.centerY == 27.5f, s"$fork.centerY should be 27.5f")
        assert(fork.size.toFloat == 5f, s"${fork.size} should be 5f")
      case _ =>
