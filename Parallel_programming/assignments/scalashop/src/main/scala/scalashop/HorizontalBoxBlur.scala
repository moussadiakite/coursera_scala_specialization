...
object HorizontalBoxBlur {
  ...
  // TODO implement using the `task` construct and the `blur` method
    val step = Math.round(src.height.toDouble / numTasks).toInt
    if(step == 0)
      blur(src, dst, 0, src.height, radius)
    else {
      val splitting_points_start = (0 until src.height by step)
      val splitting_points = splitting_points_start zip (splitting_points_start.tail ++ List(src.height))
