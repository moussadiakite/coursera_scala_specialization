...
class BlurSuite extends FunSuite {
  ...
  test("HorizontalBoxBlur.blur with integer bounds values radius 1 should correctly blur the first two rows of a 3x3 image") {
    val w = 3
    val h = 3
    val src = new Img(w, h)
    val dst = new Img(w, h)
    src(0, 0) = Int.MinValue; src(1, 0) = Int.MinValue; src(2, 0) = Int.MinValue
    src(0, 1) = Int.MinValue; src(1, 1) = Int.MinValue; src(2, 1) = Int.MinValue
    src(0, 2) = 6; src(1, 2) = 7; src(2, 2) = 8
    ...
    check(0, 0, Int.MinValue)
    check(1, 0, Int.MinValue)
    check(2, 0, Int.MinValue)
    check(0, 1, Int.MinValue)
    check(1, 1, Int.MinValue)
    check(2, 1, Int.MinValue)
    check(0, 2, 0)
    check(1, 2, 0)
    check(2, 2, 0)
  }
  ...
  test("RectangularBoxBlur.blur with radius 1 should correctly blur the first two rows of a 3x3 image") {
    val w = 3
    val h = 3
    val src = new Img(w, h)
    val dst = new Img(w, h)
    ...
    RectangularBoxBlur.blur(src, dst, (0, 0), (3, 2), 1)
  ...
  test("RectangularBoxBlur.blur with radius 2 should correctly blur the entire " +
    "4x3 image") {
    val w = 4
    val h = 3
    val src = new Img(w, h)
    ...
    RectangularBoxBlur.blur(src, dst, (0, 0), (4, 3), 2)
