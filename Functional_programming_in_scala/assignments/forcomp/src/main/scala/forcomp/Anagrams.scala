...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences =
    (w.toLowerCase groupBy (letter => letter)).toList.sorted map (pair => (pair._1, pair._2.length))
  ...
  def wordAnagrams(word: Word): List[Word] = {
    ...
    if(dictionaryByOccurrences contains occurrences) dictionaryByOccurrences(occurrences)
    else List()
  }
   ...
   */
  //def combinationsHelper(occurrences: Occurrences)

  def combinations3(occurrences: Occurrences): List[Occurrences] = List() ::
    (for {
        (character, nbOcc) <- occurrences
        newOcc <- 1 to nbOcc
        occsWithoutCharacter = (occurrences filter (other => other._1 != character))
        rest <- List() :: combinations(occsWithoutCharacter)
    } yield ((character, newOcc) :: rest).sorted)

  def combinations(occurrences: Occurrences): List[Occurrences] = occurrences match {
    case List() => List(List())
    case (character, nbOcc)::tail => {
  ...
  def adjust(occurrences: Map[Char, Int], occurence: (Char, Int)): Map[Char, Int] = {
    val (character, nboccurrences) = occurence
    val newNboccurrences = occurrences(character) - nboccurrences
    if(newNboccurrences != 0) occurrences + (character -> newNboccurrences)
